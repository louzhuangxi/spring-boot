...
public class DynamicSearchJqgridController {
    ...
    public JqgridResponse jqgridSearch(
            ...
            @RequestParam(value = "sord", required = true) String direction, RedirectAttributes redirectAttrs, Model model, HttpServletRequest request, HttpServletResponse response) {
         ...
         */
        Specification customSpecification = DynamicSpecificationUtils.bySearchFilter(
                new SearchFilter("level", SearchFilter.Operator.EQ, "1"));

        /**
         * 记录总数
         * ---
         * 无查询条件时写法:    int totalRecordsSize = JqgridJPAUtils.count(treeEntityRepository, filters);
         * 除了 jqgrid 传递过来的查询条件外，自己又附加查询条件时写法 (customSpecification):    int totalRecordsSize = JqgridJPAUtils.count(treeEntityRepository, filters,customSpecification);
         */
        int totalRecordsSize = JqgridJPAUtils.count(treeEntityRepository, filters, customSpecification);
        if (totalRecordsSize == 0)
            return new JqgridResponse(pageSize, 0, 0, Lists.newArrayList()); //构造空数据集，否则返回结果集 jqgird 解析会有问题
         ...
         */
        Page<TreeEntity> list = JqgridJPAUtils.getJqgridResponse(treeEntityRepository, currentPageNo, pageSize, sortParameter, direction, filters, customSpecification);
    ...
    public String jqgridCustomButton(@RequestParam(value = "oper", required = true) String oper, @RequestParam(value = "id", required = true) String[] ids,
        ...
        if (oper.equals("read")) {
            ...
            for (String id : ids) {
                ...
                String idstr = MyStringUtils.substringBetween(id, "\"", "\"");
