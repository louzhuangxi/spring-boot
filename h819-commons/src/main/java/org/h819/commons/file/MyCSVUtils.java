...
public class MyCSVUtils {
     ...
     *
     * @param csvFile      CSV 文件
     * @param recordNumber 指定行号（第 0 行号，第 7 行 ... 行号从 0 开始）
     * @return
     * @throws IOException
     */
     ...
     *
     * @param CSVRecord            行数据
     * @param excelCloumnAlphaName 英文名字为 AA-ZZ 之间，不区分大小写 。
     *                             Excel 显示的列名称，在 ExcelUtils 中定义,只能用于微软 Excel 软件打开 CSV 文件格式。
     *                             CSV 文件本身没有列名称属性，只是微软的 Excel 软件打开 CSV 文件的时候，Excel 软件本身加上了列名称，便于查看。
     * @return
     */
    public static String getCSVValue(CSVRecord CSVRecord, String excelCloumnAlphaName) {
        return CSVRecord.get(ArrayUtils.indexOf(MyExcelUtils.table, excelCloumnAlphaName.toUpperCase()));
