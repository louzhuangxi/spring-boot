...
package org.h819.commons.file.excel.poi.examples;
...
/**
 * Demonstrates how to add an image to a worksheet and set that image's size to
 * a specific number of milimetres irrespective of the width of the columns or
 * height of the rows. Overridden methods are provided so that the location of
 * the image - the cells row and column co-ordinates that define the top left
 * hand corners of the image - can be identified either in the familiar Excel
 * manner - A1 for instance - or using POI's methodolody of a column and row
 ...
 * from the left hand border of a cell. * How far - in terms of co-ordinate
 * positions - the image should be inset from the from the top of the cell. *
 * How far - in terms of co-ordinate positions - the right hand edge of the
 * image should protrude into a cell (measured from the cell's left hand edge to
 * the image's right hand edge). * How far - in terms of co-ordinate positions -
 * the bottm edge of the image should protrude into a row (measured from the
 * cell's top edge to the image's bottom edge). * The index of the column that
 * contains the cell whose top left hand corner should be aligned with the top
 * left hand corner of the image. * The index of the row that contains the cell
 * whose top left hand corner should be aligned with the image's top left hand
 * corner. * The index of the column that contains the cell whose top left hand
 * corner should be aligned with the image's bottom right hand corner * The
 * index number of the row that contains the cell whose top left hand corner
 ...
 * cell A1. Interestingly, it also has an effect on the image's resizing
 ...
 * is accomplised by specifying the the images bottom right hand corner should
 ...
 * the enclosing cell - irrespective of the anchor's type - but it will reduce
 * in size if the cell is made smaller.
 * 
 * ClientAnchor anchor =
 * sheet.getWorkbook().getCreationHelper().createClientAnchor();
 * 
 ...
 * parameters three and four; these I have referred to as determing the images
 * co-ordinates within the cell. They indicate that the image should occupy - in
 ...
 * how Excel will resize an image in reponse to the user increasing or
...
public class AddDimensionedImage {
	...
	public void addImageToSheet(String cellNumber, Sheet sheet,
			Drawing drawing, URL imageFile, double reqImageWidthMM,
			double reqImageHeightMM, int resizeBehaviour) throws IOException,
		...
		this.addImageToSheet(cellRef.getCol(), cellRef.getRow(), sheet,
				drawing, imageFile, reqImageWidthMM, reqImageHeightMM,
				resizeBehaviour);
	}
	 ...
	 *            identifies a cell on the worksheet. The image's top left hand
	 *            corner will be aligned with the top left hand corner of this
	 *            cell.
	 * @param rowNumber
	 *            A primtive int that contains the index number of a row on the
	 *            worksheet; POI row indices are zero based. Together with the
	 *            rowNumber parameter's value, this parameter identifies a cell
	 *            on the worksheet. The image's top left hand corner will be
	 ...
	 *            sheet.
	 * @param reqImageHeightMM
	 *            A primtive double that contains the required height of the
	 *            image in millimetres
	 * @param resizeBehaviour
	 *            A primitve int whose value will indicate how the height of the
	 *            row should be adjusted if the required height of the image is
	 *            greater than the height of the row.
	 * @return An instance of the ClientAnchorDetail class that will contain the
	...
	private ClientAnchorDetail fitImageToRows(Sheet sheet, int rowNumber,
		...
		if (rowHeightMM < reqImageHeightMM) {
			if ((resizeBehaviour == AddDimensionedImage.EXPAND_ROW)
					|| (resizeBehaviour == AddDimensionedImage.EXPAND_ROW_AND_COLUMN)) {
				row
						.setHeightInPoints((float) (reqImageHeightMM * ConvertImageUnits.POINTS_PER_MILLIMETRE));
	...
	public static void main(String[] args) {
		...
		try {
			if (args.length < 2) {
				System.err
						.println("Usage: AddDimensionedImage imageFile outputFile");
				return;
			}
			workbook = new HSSFWorkbook();
			sheet = workbook.createSheet("Picture Test");
			// Note that as the code has been ported to the SS model, the
			// following
			// would be equally as valid - workbook = new XSSFWorkbook();
			imageFile = args[0];
			outputFile = args[1];
			new AddDimensionedImage().addImageToSheet("B5", sheet, sheet
					.createDrawingPatriarch(), new File(imageFile).toURI()
					.toURL(), 100, 40,
					AddDimensionedImage.EXPAND_ROW_AND_COLUMN);
	 ...
	 * inset from the top of the cell. * Together, parameters five and six
	 * determine the column and row co-ordinates of the cell whose top left hand
	 * corner will be aligned with the image's top left hand corner. * Together,
	 * parameter seven and eight determine the column and row co-ordinates of
	 * the cell whose top left hand corner will be aligned with the images
	 * bottom right hand corner.
	 * 
	...
	public class ClientAnchorDetail {
		 ...
		 * 
		 * @param fromIndex
		 *            A primitive int that contains one of the co-ordinates (row
	...
	public static class ConvertImageUnits {
		 ...
		 * 
		 * @param widthUnits
		 *            The width of the column or the height of the row in
		 *            Excel's units.
