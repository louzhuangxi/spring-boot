		...
		jQuery(function ($) {
			...
			var setting = {
				...
				async : {
					...
					url : "${ctx}/ztree/async.html", //Ajax 获取数据的 URL 地址。第一次加载页面(此时后台确定第一次加载页面需要展示到树的第几级)和点击关闭的父节点时激发此 url。
					...
					confirm : function () {
						...
						if (input_css.val() == '') {
							...
							$.ajax({
								type : "post",
								async : false,
								url : "${ctx}/ztree/node/edit/css.html",
								data : { //传递的参数和值
									id : treeNode.id, //
									css : input_css.val()
								},
					...
					confirm : function () {
						...
						if (input_url.val() == '') {
							...
							$.ajax({
								type : "post",
								async : false,
								url : "${ctx}/ztree/node/edit/url.html",
								data : { //传递的参数和值
									id : treeNode.id, //
									url : input_url.val()
								},
					...
					confirm : function () {
						...
						if (input_standard.val() == '') {
							...
							$.ajax({
								type : "post",
								async : false,
								url : "${ctx}/ztree/node/link/standard.html",
								data : { //传递的参数和值
									id : treeNode.id, //
